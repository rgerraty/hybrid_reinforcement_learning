...
for r = runs
    ...
    choice_run_oldc=choice_run(old_choice,:);
    choice_run_newc=choice_run(new_choice,:);
    choice_run_noc=choice_run(no_choice,:);
	...
	if exist('PerformanceMem')
		...
		oldval_oldc(choice_time<0 | isnan(oldval_oldc(:,3)),:)=[];
		oldval_newc(choice_time<0 | isnan(oldval_newc(:,3)),:)=[];
	...
	if size(FB_pe_weight,1)>0
		...
		FB_pe_oldc(FB_time<0 | isnan(FB_pe_oldc(:,3)),:)=[];
		FB_pe_newc(FB_time<0 | isnan(FB_pe_newc(:,3)),:)=[];
		FB_pe_noc(FB_time<0 | isnan(FB_pe_noc(:,3)),:)=[];
		choice_Qdiff_oldc(choice_time<0 | isnan(choice_Qdiff_oldc(:,3)),:)=[];
		choice_Qdiff_newc(choice_time<0 | isnan(choice_Qdiff_newc(:,3)),:)=[];
		choice_Qdiff_noc(choice_time<0 | isnan(choice_Qdiff_noc(:,3)),:)=[];
		choice_Qchose_oldc(choice_time<0 | isnan(choice_Qchose_oldc(:,3)),:)=[];
		choice_Qchose_newc(choice_time<0 | isnan(choice_Qchose_newc(:,3)),:)=[];
		choice_Qchose_noc(choice_time<0 | isnan(choice_Qchose_noc(:,3)),:)=[];;
